...
因特网属于数据报网络，还有许多其他网络体系结构属于虚电路（ATM、帧中继的体系结构）。
...
#### UPnP
UPnP 是一种允许主机发现并配置邻近 NAT 的协议，UPnP 要求主机和 NAT 都是 UPnP 兼容的。
UPnP 能让主机应用程序为某些公共端口号请求一个 NAT 映射，如果 NAT 接受请求并生成映射，则外部的结点能主动发起 TCP 连接。
也就是说该应用程序能够向外部世界通告它。

#### NAPT UPnP 区别
NAPT 通信必须由内网的主机发起，如果外网的主机要发起通信，当 IP 数据报到达 NAT 路由器时，由于没有映射关系，NAT 路由器不知道把数据报交给哪台主机。
所以使用 NAPT 的内部网的主机无法充当服务器。

UPnP 是解决被动通信问题，在当前通信状态下再打开一个端口供对方来连接。
